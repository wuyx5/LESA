function test_findupdategamclosed()
...
fprintf('Testing original findupdategamclosed.\n');
...
for i=1:length(Xdata)
    ...
    w{i} = findphistarclosed(q2,Psi,b,Theta);
    for j=1:size(q1,3)
        v{i}(:,:,j) = q1(:,:,j)-q2(:,:,j);
    end
    
    gamupdate1{i} = findupdategamclosed(v{i},w{i},b,Theta);
end

cd(mexd);
    
fprintf('Testing MEX findupdategamclosed.\n');

for i=1:length(Xdata)
    if i == l
        continue
    end
    gamupdate2{i} = findupdategamclosed(v{i},w{i},b,Theta);
end
...
for i=1:length(Xdata)
    ...
    gamupdateerr1 = norm(gamupdate1{i}(:,:,1)-gamupdate2{i}(:,:,1))/norm(gamupdate1{i}(:,:,1));
    gamupdateerr2 = norm(gamupdate1{i}(:,:,2)-gamupdate2{i}(:,:,2))/norm(gamupdate1{i}(:,:,2));
    
    fprintf('F%d vs F%d: gamupdate_L2relerr1 = %g, gamupdate_L2relerr2 = %g\n', l, i, gamupdateerr1, gamupdateerr2);
end
